...
class _FavoritesTabState extends State<FavoritesTab>
  ...
  String _getPreviewForFavorite(
      String favId, Map<String, List<ChatMessage>> allChats) {
  ...
  DateTime _getLastTsForFavorite(
      String favId, Map<String, List<ChatMessage>> allChats) {
    final chatId = 'fav:$favId';
    final msgs = allChats[chatId] ?? [];
  ...
  String _getPreviewText(String rawContent) {
    ...
    if (rawContent.startsWith('ALBUMv1:')) {
      try {
        final list =
            jsonDecode(rawContent.substring('ALBUMv1:'.length)) as List;
  ...
  void _showDeleteConfirmation(BuildContext context, FavoriteChat fav) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'Are you sure you want to remove "${fav.title}" from your favorites?'),
        actions: [
          TextButton(
              ...
              itemBuilder: (context, i) {
                ...
                return FadeTransition(
                  opacity: Tween(begin: 0.0, end: 1.0).animate(
                    CurvedAnimation(
                      parent: _listAnimController,
                      curve: Interval(i * _staggerStep, 1.0,
                          curve: Curves.easeOut),
                    ),
                  ),
                  child: InkWell(
